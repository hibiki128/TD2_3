...
void Player::DebugImGui() {
    ...
    if (ImGui::BeginTabBar(className_.c_str())) {
        if (ImGui::BeginTabItem("パラメーター調整")) {
            ...
            ImGui::Text("反転中 : %s", isInverting_ ? "true" : "false");
            ImGui::Text("アニメーション停止中 : %s", BaseObject::GetStopAnima() ? "true" : "false");
...
void Player::BaseUpdate() {
    ...
    if (isInverting_ || mapChipField_->IsAnyChipAnimating()) {
        BaseObject::SetStopAnima(true);
    } else {
        BaseObject::SetStopAnima(false);
    }
    BaseObject::Update();
}
...
Player::CollisionMapInfo Player::GetMapCollisionInfo(bool title) {
    ...
    const float overlapOffsetX = (kWidth / 2) - 0.15f;
    const float overlapOffsetY = (kHeight / 2) - 0.15f;
