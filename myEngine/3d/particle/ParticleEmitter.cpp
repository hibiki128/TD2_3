...
void ParticleEmitter::Initialize(std::string name, std::string data, std::string filePath) {
...
void ParticleEmitter::LoadFromJson(std::string name) {
    if (!name.empty()) {
        ...
        count_ = data_->Load<int>("count", 1);
        emitFrequency_ = data_->Load<float>("emitFrequency", 0.1f);
        lifeTimeMin_ = data_->Load<float>("lifeTimeMin", 1.0f);
        lifeTimeMax_ = data_->Load<float>("lifeTimeMax", 3.0f);
        alphaMin_ = data_->Load<float>("alphaMin", 1.0f);
        alphaMax_ = data_->Load<float>("alphaMax", 1.0f);
        scaleMin_ = data_->Load<float>("scaleMin", 1.0f);
        scaleMax_ = data_->Load<float>("scaleMax", 1.0f);
        velocityMin_ = data_->Load<Vector3>("velocityMin", {-1.0f, -1.0f, -1.0f});
        velocityMax_ = data_->Load<Vector3>("velocityMax", {1.0f, 1.0f, 1.0f});
        startScale_ = data_->Load<Vector3>("startScale", {1.0f, 1.0f, 1.0f});
        endScale_ = data_->Load<Vector3>("endScale", {0.0f, 0.0f, 0.0f});
        startAcce_ = data_->Load<Vector3>("startAcce", {1.0f, 1.0f, 1.0f});
        endAcce_ = data_->Load<Vector3>("endAcce", {1.0f, 1.0f, 1.0f});
        startRote_ = data_->Load<Vector3>("startRote", {0.0f, 0.0f, 0.0f});
        endRote_ = data_->Load<Vector3>("endRote", {0.0f, 0.0f, 0.0f});
        rotateVelocityMin_ = data_->Load<Vector3>("rotateVelocityMin", {-0.07f, -0.07f, -0.07f});
        rotateVelocityMax_ = data_->Load<Vector3>("rotateVelocityMax", {0.07f, 0.07f, 0.07f});
        allScaleMin_ = data_->Load<Vector3>("allScaleMin", {0.0f, 0.0f, 0.0f});
        allScaleMax_ = data_->Load<Vector3>("allScaleMax", {1.0f, 1.0f, 1.0f});
        isRandomScale_ = data_->Load<bool>("isRandomScale", false);
        isAllRamdomScale_ = data_->Load<bool>("isAllRamdomScale", false);
        isRandomColor_ = data_->Load<bool>("isRandomColor", false);
        isRandomRotate_ = data_->Load<bool>("isRandomRotate", false);
        isVisible_ = data_->Load<bool>("isVisible", true);
        isBillBoard_ = data_->Load<bool>("isBillBoard", false);
        isActive_ = data_->Load<bool>("isActive", false);
        isAcceMultiply_ = data_->Load<bool>("isAcceMultiply", false);
        isSinMove_ = data_->Load<bool>("isSinMove", false);
        isFaceDirection_ = data_->Load<bool>("isFaceDirection", false);
        fileName_ = data_->Load<std::string>("fileName", fileName_);
        texturePath_ = data_->Load<std::string>("texturePath", texturePath_);
    }
}
#pragma endregion